...
def run_model (train_data_indices, pool_sample_indices):
  ...
  pool_sample_predictions = predict_with_uncertainty(f, np.expand_dims(X_train_All[pool_sample_indices], axis=1), n_iter=100)
...
for e in range(num_experiments):
  ...
  sample_ind = np.random.choice(pool_indices, sample_size, replace=False)
  model_results = run_model(train_data_indices, sample_ind)
  exp_acc.append(model_results[0])
  for acq in range(num_acquisitions):
    acq_fn = np.absolute(np.subtract(model_results[1], 0.5))
    acq_ind_ind = np.argmin(acq_fn)
    acq_ind = sample_ind[acq_ind_ind] 
    ...
    np.save(folder_path+'MaxEntAcqAcc.npy', np.array(all_acc))
    np.save(folder_path+'MaxEntAcqInd.npy', np.array(all_tr_ind))
print('All Acc: ' + str(all_acc))
