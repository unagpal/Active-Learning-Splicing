...
def ei_det_acquisition_fn_cnn (univ_covariance, num_pool_samples, num_training_samples):
    ...
    for new_pt_ind in range(num_pool_samples):
        ...
        print("all eig: " + str(np.linalg.eigh(updated_univ_covariance)[0]))
        print("max eig: " + str(np.amax(np.linalg.eigh(updated_univ_covariance)[0])))
    return [all_acq_values, original_log_det] 

def ei_max_eig_acquisition_fn_cnn (univ_covariance, num_pool_samples, num_training_samples):
    all_acq_values = np.zeros(num_pool_samples)
    original_max_eig = np.amax(np.linalg.eigh(univ_covariance)[0])
    for new_pt_ind in range(num_pool_samples):
        current_index = num_training_samples + new_pt_ind
        covariance_vector = np.expand_dims(univ_covariance[current_index, :], 1)
        updated_univ_covariance = np.subtract(univ_covariance, np.matmul(covariance_vector, covariance_vector.T)\
            /(univ_covariance[current_index,current_index]))
        all_acq_values[new_pt_ind] = np.amax(np.linalg.eigh(updated_univ_covariance)[0])
    return [all_acq_values, original_max_eig] 
...
def get_acquisition_fn (model, X_train_sample, X_cand, num_masks, tau_inv_prop):
    ...
    print('final output cov shape: ' + str(final_output_covariance.shape))
    return ei_max_eig_acquisition_fn_cnn(final_output_covariance, len(X_cand), len(X_train_sample)) 
...
#exp_mse_rand = []
#exp_mse_expimprovement = np.load(os.path.expanduser("~/EICNNMaxEigReduxTest_TauInv_MSEInd2.npy")).tolist() 
...
filepath_expimprovement_mse = os.path.expanduser("~/TAUINVPOINT5_EICNNMaxEigReduxTest_TauInv_MSEInd2.npy")
filepath_expimprovement_det_univ_original = os.path.expanduser("~/EICNNMaxEigReduxTest_TauInv_MaxEigOrig_Ind2.npy")
filepath_expimprovement_acq_val = os.path.expanduser("~/EICNNMaxEigReduxTest_TauInv_AcqValInd2.npy")
filepath_expimprovement_det_univ_new = os.path.expanduser("~/EICNNMaxEigReduxTest_TauInv_MaxEigNew_Ind2.npy")
all_train_indices = np.load(os.path.expanduser('~/trainindices2.npy'))
filepath_indices = os.path.expanduser("~/EICNNMaxEigReduxTest_TauInv_TrInd_Ind2.npy")
filepath_det_errors = os.path.expanduser("~/EICNNDetReduxTest_TauInv_OffByOneErrors.npy")
...
#train_data_indices_expimprovement = np.load(filepath_indices).tolist()
...
ei_acquired_values = []
all_original_max_eig = []
all_updated_max_eig = []
#ei_acquired_values = np.load(filepath_expimprovement_acq_val).tolist()
#all_original_max_eig = np.load(filepath_expimprovement_det_univ_original).tolist()
#all_updated_max_eig = np.load(filepath_expimprovement_det_univ_new).tolist()
#Acquisition Loop
for acq in range(num_acquisitions):
    ...
    """
    #tau_inv = (2 * (size_train+acq) * Weight_Decay)/((1-dropout_prob)*lengthscale**2)
    ...
    acq_fn_results = get_acquisition_fn(model_expimprovement, X_train_fn, X_pool_fn, acq_fn_dropout_iterations, tau_inv_proportion)
    acq_fn_values = acq_fn_results[0] 
    all_original_max_eig.append(acq_fn_results[1])
    ...
    all_updated_max_eig.append(np.amax(np.linalg.eigh(np.cov(all_univ_pred_new))[0]))
    print('Size train: ' + str(len(train_data_indices_expimprovement)))
    print("Updated all eig shape: " + str(np.linalg.eigh(np.cov(all_univ_pred_new))[0].shape))
    ...
    print('Det orig: ' + str(all_original_max_eig[-1]))
    print('Det new: ' + str(all_updated_max_eig[-1]))
    print("Acq Itr " + str(acq) + ", ExpImprovement Univ MSE: " + str(exp_mse_expimprovement[-1]) + ', EI Value: ' + str(ei_acquired_values[-1]) + ', Updated MaxEig(Cov Univ): ' + str(all_updated_max_eig[-1])+', MaxEig Reduction: ' + str(all_original_max_eig[-1]-all_updated_max_eig[-1]))
    print("len saved mse: " + str(len(exp_mse_expimprovement)))
    print('len saved ind: ' + str(len(train_data_indices_expimprovement)))
    ...
    #np.save(filepath_expimprovement_det_univ_original, np.array(all_original_max_eig))
    #np.save(filepath_indices, np.array(train_data_indices_expimprovement))
    #np.save(filepath_expimprovement_det_univ_new, np.array(all_updated_max_eig))
