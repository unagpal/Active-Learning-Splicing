...
def get_acquisition_fn(pool_forward_pass, num_tr_pts, tau_inv_prop):
  ...
  tau_inv = np.trace(pool_cov)*tau_inv_prop/(pool_cov.shape[0])
  pool_cov = pool_cov + tau_inv * np.identity(pool_cov.shape[0])
  num_pool_pts = int(pool_cov.shape[0])-num_tr_pts
  ei_acq_values = np.zeros(num_pool_pts)
  for new_pt_ind in range(num_pool_pts):
    cov_vector = pool_cov[num_tr_pts+new_pt_ind,:]
    ei_acq_values[new_pt_ind] = np.sum(np.square(cov_vector))/(pool_cov[num_tr_pts+new_pt_ind,num_tr_pts+new_pt_ind]) 
  normalized_ei_acq_values = ei_acq_values/np.amax(ei_acq_values)
  #Calculating MaxEnt Acquisition Function Values
  ...
  maxent_acq_values = np.multiply(-1, np.add(ent_term_1, ent_term_2))[num_tr_pts:]
  print('maxent acq values : ' + str(maxent_acq_values))
  print('num nonzero maxent acq values: ' + str(len(maxent_acq_values[maxent_acq_values!=0.0])))
  print('highest value of EI Ent: ' + str(np.amax(normalized_ei_acq_values+maxent_acq_values)))
  print('class prob for maxent: ' + str(all_prob_pred_class_1[num_tr_pts:][np.argmax(maxent_acq_values)]))
  print('class prob for eient: ' + str(all_prob_pred_class_1[num_tr_pts:][np.argmax(normalized_ei_acq_values+maxent_acq_values)]))
  return normalized_ei_acq_values+maxent_acq_values
